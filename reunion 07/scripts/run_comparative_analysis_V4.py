...
try:
    # AJUSTE DE RUTA: Sube dos niveles para llegar a la raíz del proyecto
...
def analizar_fronteras_y_puentes_V4(df_original: pd.DataFrame, df_nodes_clusters: pd.DataFrame,
    ...
    """
    V4: Analiza fronteras y puentes diferenciando el destino de los links externos.
    - Caracteriza nodos frontera según sus conexiones a NEUTRALES vs. OTRO CLUSTER.
    - Analiza nodos puente neutrales y su equilibrio.
    """
    ...
    for cluster_id, cluster_name in cluster_map.items():
        ...
        for _, row in aristas_externas.iterrows():
            nodo_frontera = None
            destino_cluster = None

            if row['FROM_CLUSTER'] == cluster_id:
                nodo_frontera = row['FROM_NODE_norm']
                destino_cluster = row['TO_CLUSTER']
            elif row['TO_CLUSTER'] == cluster_id:
                nodo_frontera = row['TO_NODE_norm']
                destino_cluster = row['FROM_CLUSTER']

            if nodo_frontera:
                frontera.add(nodo_frontera)
                if destino_cluster == 0:  # Conexión hacia un Neutral
                    conex_vs_neutral[nodo_frontera] += 1
                elif destino_cluster == otro_cluster_id:  # Conexión hacia el Otro Cluster
    ...
    for nodo_neutral in nodos_neutrales:
        ...
        if 1 in conexiones_a_clusters and -1 in conexiones_a_clusters:
            conex_s1 = conexiones_a_clusters.get(1, 0)
            conex_s2 = conexiones_a_clusters.get(-1, 0)
            total_conexiones = conex_s1 + conex_s2
            
            resultados_analisis.append({
                'Dataset': nombre_dataset, 'Algoritmo': nombre_algoritmo, 'Tipo_Analisis': 'Neutral_Puente',
                'Nodo': nodo_neutral,
                'Conexiones_S1': conex_s1,
                'Conexiones_S2': conex_s2,
                'Conexiones_Totales': total_conexiones,
                'Sesgo_Equilibrio': (conex_s1 - conex_s2) / total_conexiones if total_conexiones > 0 else 0
            })
...
def ejecutar_experimento_individual(nombre_dataset, df_dataset, config_base, default_cols, cluster_map):
    """
    Función wrapper para ejecutar un único pipeline y su análisis posterior.
    """
    ...
    if df_nodes_results is not None and not df_nodes_results.empty:
        # LLAMADA A LA NUEVA FUNCIÓN V4
        fronteras_encontradas = analizar_fronteras_y_puentes_V4(
...
def run_comparative_analysis():
    """
    Función principal que carga datos, define experimentos y los ejecuta en paralelo.
    """
    logging.info("--- INICIANDO EXPERIMENTO COMPARATIVO (V4 CON ANÁLISIS DE DESTINO) ---")
    ...
    resultados_analisis = [item for res in resultados_paralelos if res and res[1] for item in res[1]]

    df_resultados_core = pd.DataFrame(resultados_core)
    df_resultados_analisis = pd.DataFrame(resultados_analisis)
    ...
    # CAMBIO: Nuevos nombres de archivo para V4
    path_metricas = os.path.join(results_dir, 'exp_metricas_polarizacion_V4.csv')
    ...
    df_resultados_analisis.to_csv(path_analisis, index=False, sep=';', encoding='utf-8-sig')
    
    logging.info("--- Resultados V4 guardados exitosamente ---")
    logging.info(f"Métricas de polarización: {path_metricas}")
    logging.info(f"Análisis de Fronteras y Puentes: {path_analisis}")
